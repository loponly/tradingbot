...
from tradingbot.constants import HYPEROPT_LOSS_BUILTIN
...
def check_int_positive(value: str) -> int:
    try:
        ...
        raise ArgumentTypeError(
            f"{value} is invalid for this parameter, should be a positive interger value"
        )
    return unit
...
AVAILABLE_CLI_OPTIONS = {
    ...
    Arg(
        '--logfile',
        help=
        "Log to the file specified. Special values are: 'syslog', 'journald'. "
    ...
    Arg(
        ...
        '--config',
        help=
        f'Specify configuration file (default: `userdir/{constants.DEFAULT_CONFIG}` '
    ...
    Arg(
        '--db-url',
        help=
        f'Override trades database URL, this is useful in custom deployments '
    ...
    Arg(
        '--dry-run',
        help=
        'Enforce dry-run for trading (removes Exchange secrets and simulates trades).',
        action='store_true',
    ),
    # Optimize common
    "timeframe":
    ...
    Arg(
        '--fee',
        help=
        'Specify fee ratio. Will be applied twice (on trade entry and exit).',
        type=float,
        metavar='FLOAT',
    ),
    # Edge
    ...
    Arg(
        '--stoplosses',
        help=
        'Defines a range of stoploss values against which edge will assess the strategy. '
    ...
    Arg(
        '--hyperopt-path',
        help=
        'Specify additional lookup path for Hyperopt and Hyperopt Loss functions.',
        metavar='PATH',
    ),
    "epochs":
    Arg(
    ...
    Arg(
        ...
        choices=[
            'all', 'buy', 'sell', 'roi', 'stoploss', 'trailing', 'default'
        ],
        nargs='+',
        default='default',
    ),
    ...
    Arg(
        '--no-color',
        help=
        'Disable colorization of hyperopt results. May be useful if you are '
    ...
    Arg(
        '--random-state',
        help=
        'Set random state to some positive integer for reproducible hyperopt results.',
        type=check_int_positive,
        metavar='INT',
    ),
    "hyperopt_min_trades":
    Arg(
        '--min-trades',
        help=
        "Set minimal desired number of trades for evaluations in the hyperopt "
    ...
    Arg(
        '--hyperopt-loss',
        help=
        'Specify the class name of the hyperopt loss function class (IHyperOptLoss). '
    ...
    Arg(
        '--dl-trades',
        help=
        'Download trades instead of OHLCV data. The bot will resample trades to the '
    ...
    Arg('--data-format-ohlcv',
        help=
        'Storage format for downloaded candle (OHLCV) data. (default: `%(default)s`).',
        choices=constants.AVAILABLE_DATAHANDLERS,
        default='json'),
    "dataformat_trades":
    Arg('--data-format-trades',
        help=
        'Storage format for downloaded trades data. (default: `%(default)s`).',
        choices=constants.AVAILABLE_DATAHANDLERS,
        default='jsongz'),
    "exchange":
    Arg(
    ...
    Arg(
        '--erase',
        help=
        'Clean all existing data for the selected exchange/pairs/timeframes.',
        action='store_true',
    ),
    # Templating options
    "template":
    ...
    Arg(
        '--indicators2',
        help=
        'Set indicators from your strategy you want in the third row of the graph. '
    ...
    Arg(
        '--plot-limit',
        help=
        'Specify tick limit for plotting. Notice: too high values cause huge files. '
        'Default: %(default)s.',
        type=check_int_positive,
        metavar='INT',
        default=750,
