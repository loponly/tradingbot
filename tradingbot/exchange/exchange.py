...
class Exchange:
    ...
    def __init__(self, config: Dict[str, Any], validate: bool = True) -> None:
        ...
        ccxt_config = deep_merge_dicts(
            exchange_config.get('ccxt_sync_config', {}), ccxt_config)

        self._api = self._init_ccxt(exchange_config, ccxt_kwargs=ccxt_config)

        ccxt_async_config = self._ccxt_config.copy()
        ccxt_async_config = deep_merge_dicts(
            exchange_config.get('ccxt_config', {}), ccxt_async_config)
        ccxt_async_config = deep_merge_dicts(
    ...
    def __del__(self):
        ...
        if self._api_async and inspect.iscoroutinefunction(
    ...
    def get_markets(self,
        ...
        if base_currencies:
            markets = {
                k: v
                for k, v in markets.items() if v['base'] in base_currencies
            }
        if quote_currencies:
            markets = {
                k: v
                for k, v in markets.items() if v['quote'] in quote_currencies
            }
        if pairs_only:
            markets = {
                k: v
                for k, v in markets.items() if self.market_is_tradable(v)
            }
        if active_only:
    ...
    def market_is_tradable(self, market: Dict[str, Any]) -> bool:
        ...
        return (len(symbol_parts) == 2 and len(symbol_parts[0]) > 0
                and len(symbol_parts[1]) > 0
                and symbol_parts[0] == market.get('base')
                and symbol_parts[1] == market.get('quote'))

    def klines(self,
               pair_interval: Tuple[str, str],
               copy: bool = True) -> DataFrame:
        if pair_interval in self._klines:
            return self._klines[pair_interval].copy(
            ) if copy else self._klines[pair_interval]
        else:
            return DataFrame()

    def set_sandbox(self, api: ccxt.Exchange, exchange_config: dict,
                    name: str) -> None:
        if exchange_config.get('sandbox'):
            if api.urls.get('test'):
                ...
                raise OperationalException(
                    f'Exchange {name} does not provide a sandbox api')
        ...
        for pair in pairs:
            ...
            if self.markets and pair not in self.markets:
                ...
                # It can also be a string or similar ... so we need to verify that first.
                  ...
                  and self.markets[pair].get('info', {}).get(
                      'IsRestricted', False)):
            ...
            if (self._config['stake_currency']
                    and self.get_pair_quote_currency(pair) !=
    ...
    def buy(self, pair: str, ordertype: str, amount: float, rate: float,
        ...
        if self._config['dry_run']:
            dry_order = self.dry_run_order(pair, ordertype, "buy", amount,
                                           rate)
            return dry_order
    ...
    def stoploss_adjust(self, stop_loss: float, order: Dict) -> bool:
        ...
        """
        raise OperationalException(
            f"stoploss is not implemented for {self.name}.")
    ...
    def get_historic_ohlcv(self, pair: str, timeframe: str,
        ...
        return asyncio.get_event_loop().run_until_complete(
            self._async_get_historic_ohlcv(pair=pair,
                                           timeframe=timeframe,
                                           since_ms=since_ms))
    ...
    async def _async_get_historic_ohlcv(self, pair: str, timeframe: str,
        ...
        logger.debug(
            "one_call: %s msecs (%s)", one_call,
            arrow.utcnow().shift(seconds=one_call //
                                 1000).humanize(only_distance=True))
        input_coroutines = [
            self._async_get_candle_history(pair, timeframe, since)
            for since in range(since_ms,
                               arrow.utcnow().timestamp * 1000, one_call)
        ]

        results = await asyncio.gather(*input_coroutines,
    ...
    def refresh_latest_ohlcv(
            self,
            pair_list: ListPairsWithTimeframes) -> List[Tuple[str, List]]:
        """
        ...
        """
        logger.debug("Refreshing candle (OHLCV) data for %d pairs",
                     len(pair_list))
    ...
    def _now_is_time_to_refresh(self, pair: str, timeframe: str) -> bool:
        ...
        return not ((self._pairs_last_refresh_time.get(
            (pair, timeframe), 0) + interval_in_sec) >=
                    arrow.utcnow().timestamp)
    ...
    async def _async_get_trade_history(
        ...
        if self._trades_pagination == 'time':
            return await self._async_get_trade_history_time(pair=pair,
                                                            since=since,
                                                            until=until)
        elif self._trades_pagination == 'id':
            return await self._async_get_trade_history_id(pair=pair,
                                                          since=since,
                                                          until=until,
                                                          from_id=from_id)
        else:
    ...
    def get_historic_trades(self,
        ...
        if not self.exchange_has("fetchTrades"):
            raise OperationalException(
                "This exchange does not suport downloading Trades.")

        return asyncio.get_event_loop().run_until_complete(
            self._async_get_trade_history(pair=pair,
                                          since=since,
                                          until=until,
                                          from_id=from_id))
    ...
    def check_order_canceled_empty(self, order: Dict) -> bool:
        ...
        """
        return order.get('status') in (
            'closed', 'canceled') and order.get('filled') == 0.0
    ...
    def cancel_order_with_result(self, order_id: str, pair: str,
                                 amount: float) -> Dict:
        """
    ...
    def fetch_order_or_stoploss_order(self,
                                      order_id: str,
                                      pair: str,
    ...
    def fetch_l2_order_book(self, pair: str, limit: int = 100) -> dict:
        ...
        """
        limit1 = self.get_next_limit_in_list(limit,
    ...
    def get_trades_for_order(self, order_id: str, pair: str,
        ...
        try:
            ...
            my_trades = self._api.fetch_my_trades(
                pair,
                int((since.replace(tzinfo=timezone.utc).timestamp() - 5) *
                    1000))
            matched_trades = [
    ...
    def get_fee(self,
                ...
                amount: float = 1,
                price: float = 1,
                taker_or_maker: str = 'maker') -> float:
        try:
    ...
    def calculate_fee_rate(self, order: Dict) -> Optional[float]:
        ...
        if fee_curr in self.get_pair_base_currency(order['symbol']):
            ...
            return round(
                order['fee']['cost'] /
                safe_value_fallback2(order, order, 'filled', 'amount'), 8)
        elif fee_curr in self.get_pair_quote_currency(order['symbol']):
            # Quote currency - divide by cost
            return round(order['fee']['cost'] /
                         order['cost'], 8) if order['cost'] else None
        else:
            ...
            try:
                ...
                fee_to_quote_rate = safe_value_fallback2(
                    tick, tick, 'last', 'ask')
                return round(
                    (order['fee']['cost'] * fee_to_quote_rate) / order['cost'],
                    8)
            except ExchangeError:
                return None

    def extract_cost_curr_rate(
            self, order: Dict) -> Tuple[float, str, Optional[float]]:
        """
        ...
        """
        return (order['fee']['cost'], order['fee']['currency'],
