...
from tradingbot.constants import DEFAULT_DATAFRAME_COLUMNS, DEFAULT_TRADES_COLUMNS

logger = logging.getLogger(__name__)


def ohlcv_to_dataframe(ohlcv: list,
                       timeframe: str,
                       pair: str,
                       *,
...
def clean_ohlcv_dataframe(data: DataFrame,
    ...
    :return: DataFrame
    """
    # group by index and aggregate results to elimane duplicate ticks

    data = data.groupby(by='data', as_index=False, sort=True).agg({
        'open':
        'first',
        'high':
        'max',
...
def ohlcv_fill_up_missing_data(dataframe: DataFrame, timeframe: str,
    ...
    """
    from tradingbot.exchange import timeframe_to_minutes
    ohlcv_dict = {
        'open': 'first',
        'high': 'max',
        'low': 'min',
...
def trim_dataframe(df: DataFrame,
                   timerange,
                   df_date_col: str = 'date') -> DataFrame:
    """
...
def convert_trades_format(config: Dict[str, Any], convert_from: str,
                          convert_to: str, erase: bool):
    """
...
def convert_ohlcv_format(config: Dict[str, Any], convert_from: str,
                         convert_to: str, erase: bool):
    """
    ...
    if 'pairs' not in config:
        ...
        for timeframe in timeframes:
            config['pairs'].extend(
                src.ohlcv_get_pairs(config['datadir'], timeframe))
    ...
    for timeframe in timeframes:
        for pair in config['pairs']:
            ...
            if len(data) > 0:
                ...
                if erase and convert_from != convert_to:
                    logger.info(
                        f"Deleting source data for {pair} / {timeframe}")
