...
class HDF5DataHandler(IDataHandler):
    ...
    @classmethod
    def ohlcv_get_available_data(cls,
                                 datadir: Path) -> ListPairsWithTimeframes:
        """
        ...
        """
        _tmp = [
            re.search(r'^([a-zA-Z_]+)\-(\d+\S+)(?=.h5)', p.name)
    ...
    def ohlcv_get_pairs(cls, datadir: Path, timeframe: str) -> List[str]:
        ...
        _tmp = [
            re.search(r'^(\S+)(?=\-' + timeframe + '.h5)', p.name)
            for p in datadir.glob(f"*{timeframe}.h5")
        ]
        # Check if regex found something and only return these results
        return [match[0].replace('_', '/') for match in _tmp if match]

    def ohlcv_store(self, pair: str, timeframe: str,
        ...
        ds.put(key,
               _data.loc[:, self._columns],
               format='table',
               data_columns=['date'])

        ds.close()

    def _ohlcv_load(self,
                    pair: str,
                    timeframe: str,
    ...
    def ohlcv_append(self, pair: str, timeframe: str,
                     data: pd.DataFrame) -> None:
        """
    ...
    def trades_get_pairs(cls, datadir: Path) -> List[str]:
        ...
        """
        _tmp = [
            re.search(r'^(\S+)(?=\-trades.h5)', p.name)
    ...
    def trades_store(self, pair: str, data: TradeList) -> None:
        ...
        ds = pd.HDFStore(self._pair_trades_filename(self._datadir, pair),
                         ...
                         complib='blosc')
        ds.put(key,
               pd.DataFrame(data, columns=DEFAULT_TRADES_COLUMNS),
               format='table',
    ...
    def _trades_load(self,
                     pair: str,
                     timerange: Optional[TimeRange] = None) -> TradeList:
        """
    ...
    @classmethod
    def _pair_data_filename(cls, datadir: Path, pair: str,
                            timeframe: str) -> Path:
